...
export function MediaCard({ expanded = false, onToggleExpand }: Props) {
  ...
  if (expanded) {
    return (
      <div className="relative flex h-full w-full flex-col gap-2 rounded-[24px] bg-[var(--panel)] p-2.5 shadow-sm ring-1 ring-black/5 animate-fade-in">
        <audio ref={audioRef} src={cur.src} onEnded={() => skip(1)} preload="auto" />
        <div className="relative h-[126px] overflow-hidden rounded-[16px] bg-gradient-to-br from-sky-400 via-indigo-500 to-fuchsia-500">
          <div className="absolute inset-0 bg-black/20" />
          <div className="absolute left-3 top-2 text-[30px] font-extrabold italic tracking-tight text-white drop-shadow">FLOWERS</div>
          <div className="absolute left-3 top-[44px] text-[9px] font-semibold uppercase tracking-[0.18em] text-white/90">MILEY CYRUS</div>
        </div>
        <div className="rounded-[12px] bg-[var(--panel-soft)] px-2 py-1.5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto rounded-xl bg-[var(--panel-soft)] p-1.5">
          {PLAYLIST.map((t, i) => (
            <button
              key={i}
              onClick={() => { setTrackIdx(i); setPlaying(true); }}
              className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[10px] ${i === trackIdx ? "bg-[var(--brand)]/15 font-bold text-[var(--brand)]" : "hover:bg-black/5"}`}
            >
